...
class AddonData:
    ...
    def get_data(context=None) -> 'AddonData':
        ...
        if not context:
            return get_prefs(bpy.context).brush_manager
        return get_prefs(context).brush_manager.data

    @classmethod
    def get_data_by_ui_mode(cls, context: Context | str = None) -> 'AddonDataByMode':
        return cls.get_data(context).sculpt
    ...
    @classmethod
    def get_data_by_ctx_mode(cls, context=None) -> Union['AddonDataByMode', None]:
        if not context:
            context = bpy.context
